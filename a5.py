...
class Board:
    ...
    def find_most_constrained_cell(self) -> Tuple[int, int]:
        ...
        for i, col in enumerate(self.rows):
            for j, cell in enumerate(col):
                if isinstance(cell, list) and len(cell) < min_p:
                    min_p = len(cell)
                    row = i
                    column = j
        return (row, column)
    ...
    def update(self, row: int, column: int, assignment: int) -> None:
        ...
        self.num_nums_placed += 1 # Placing one more number
...
def DFS(state: Board) -> Board:
    ...
    if state.failure_test():
        return None

    current = copy.deepcopy(state)
    for value in current.rows[row][col]:
...
def BFS(state: Board) -> Board:
    ...
    while not queue.is_empty():
        ...
        row, col = current.find_most_constrained_cell()
        for value in current.rows[row][col]:
            new = copy.deepcopy(current)
            new.update(row, col, value)
            queue.push(new)
